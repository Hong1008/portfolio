...
Apache POI의 저수준 API를 직접 사용하면 Workbook, Sheet, Row, Cell과 스타일 객체를 반복적으로 관리해야 했습니다. 대용량 데이터와 많은 병합 영역을 다룰 때는 API 복잡도뿐 아니라 처리량과 메모리 사용 특성도 함께 고려해야 했습니다.

### 판단

사용자 코드는 Kotlin DSL에만 의존하도록 하고, 실제 엑셀 엔진은 `ExcelDriver` 인터페이스 뒤로 분리했습니다. 스트리밍에서 허용되지 않는 역방향 행 접근과 동일 시트 동시 쓰기는 묵시적으로 허용하지 않고 즉시 실패하도록 했습니다.

### 역할

개인 프로젝트로 API와 Scope 계층 설계, POI·FastExcel 드라이버 구현, 벤치마크와 최적화, 배포까지 전 과정을 수행했습니다.

### 검증

OpenJDK 21, 512MB heap, G1GC 환경에서 JMH 벤치마크를 구성했습니다. FastExcel DSL 오버헤드를 16.3%에서 3.1%로 낮췄고, 병합 처리량은 3.36에서 17.42 ops/s로 개선했습니다.

### 한계

POI의 병합 중복 검증을 우회한 최적화는 잘못된 병합 요청을 사전에 방지한다는 전제가 필요합니다. 또한 병합 메타데이터는 파일 작성이 끝날 때까지 메모리에 남습니다.
