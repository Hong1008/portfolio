...
summary: "PG·인앱 결제 승인이 끝나도 내부 재화 제공은 실패할 수 있는 운영 결제 시스템을 다뤘습니다. 두 완료 시점을 주문 상태로 나누고 PURCHASED와 Redis 정보가 남은 주문만 재처리하도록 판단했습니다. 결제 API·상태 흐름·복구 배치를 구현하고 상태·TTL·DB 제약의 방어 범위를 대조했습니다. 승인 후 DB 저장 실패는 복구하지 못한다는 한계까지 구분해 상태와 복구 경계를 설계한 사례입니다."
...
caseSummary:
  context: "PG와 Google Play·Apple 인앱 결제를 하나의 서비스 흐름으로 연결한 운영 결제 시스템입니다."
  ...
  demonstrates: "외부 승인과 내부 완료를 상태로 분리하고, 복구 가능한 실패와 남는 복구 공백을 함께 설명하는 역량을 보여줍니다."
